...
<div class="row row-eq-height vlog-cats row row-eq-height-eq-height vlog-slider vlog-module module-cats col-lg-12 col-md-12 col-sm-12 <?php echo esc_attr( $module['css_class'] ); ?>" id="vlog-module-<?php echo $catkey; ?>" data-col="12">
    <?php $first=true; ?>
	<?php foreach($slides['first'] as $sfirst => $post ): setup_postdata( $post ); ?>
			...
			<?php if($first && $sfirst == 0): ?> <!--Close123345345-->
				...
				<div class="vlog-module module-cats col-lg-12 col-md-12 col-sm-12">
					<div class="row row-eq-height vlog-cats col-md-8 col-sm-8">
					...
					<div class="entry-meta description" style="color:#FFF;"><?php echo substr(strip_tags($post->post_content), 0, 400);?>
						<div class="entry-category video-time"><?php echo komaindo_front_get_video_time($post->ID); ?></div>
					</div>
					</div>
					<div class="row row-eq-height vlog-cats col-md-4 col-sm-4">
			<?php else: ?>
			        <div class="row row-eq-height vlog-cats col-md-12 col-sm-12">
					<div class="entry-image">
