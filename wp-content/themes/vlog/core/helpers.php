...
if ( !function_exists( 'vlog_get_home_category' ) ):
	function vlog_get_home_category() {
		$categories = komando_front_get_home_three_piece_categories();  
		$carousel = array();

		foreach($categories as $category_home ){ 
			$cat = $category_home['term'];
		
			$posts = array();
			$posts_orders = array();
			$carousel_three = array();
			$reorder_obj = new ReOrderPostWithinCategory();
			$posts_orders = $reorder_obj->get_posts_catgegory_order_based('post', $cat->term_id, 'category');
			
			$index=1;
            if(isset($posts_orders) && count($posts_orders) > 0){
			
			foreach($posts_orders as $post){
				$posts[] = get_post($post);   
			}
			$counter = 1;
