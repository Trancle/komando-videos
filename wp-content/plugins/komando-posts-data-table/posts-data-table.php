 ...
 *
 * @package   Komando Override: Posts_Data_Table(For Search video Library)
 ...
 *
 * @wordpress-plugin
 * Plugin Name:       komondo-admin:Posts Table with Search & Sort
...
class Posts_Data_Table_Plugin {
    ...
    private $shortcode_defaults = array(
        //'columns' => 'selectinsert,featuredimage,title,content,date,author,category',
		'columns' => 'selectinsert,featuredimage,title,date,author',
        'rows_per_page' => 20,
        'sort_by' => 'date',
        'sort_order' => '',
        'category' => '',
        ...
        'content_length' => 35,
        'scroll_offset' => 15,
		'post' => 'videos_library'//'episode_shows'
    );
	...
	public function register_styles() {
        wp_enqueue_style( 'jquery-data-tables', plugins_url( 'assets/css/datatables.css', __FILE__ ), array(), '1.10.12' );
    ...
    public function register_scripts() {
        ...
        //Fpr custom Js code at admin
		wp_enqueue_script( 'custom', plugins_url( 'assets/js/custom.js', __FILE__ ), array( 'jquery-data-tables' ), POSTS_DATA_TABLE_VERSION, true );
    ...
    private function get_posts_data_table( $args ) {
        ...
        if ( is_array( $all_posts_curr_lang ) && $all_posts_curr_lang ) {  // if we have posts
            ...
            foreach ( (array) $all_posts_curr_lang as $_post ) {
				...
				$featuredimage = get_the_post_thumbnail($_post->ID, array(100,100)); //'thumbnail');
                
				
                // Format title
                //$title = sprintf( '<a href="%1$s">%2$s</a>', get_permalink($_post), get_the_title( $_post ) );
				$title = sprintf( '%2$s', get_permalink($_post), get_the_title( $_post ) );
                ...
                $post_data_trans = array( 
                    '{id}' => $_post->ID, 
					'{selectinsert}' => '<br/><br/>' . $this->selectinsert_url($_post->ID),
                    '{title}' => $title, 
            ...
            $newvideo = '<a class="newvideo" href="' . admin_url() .'/post-new.php?post_type=videos_library" target="_top"> Add new video</a>';
            $output = sprintf( 
                '<table '
                    . 'id="posts-table-%1$u" '
                    . 'class="%2$s" '
		...
		if(empty($output)){
		  ...
		  $output = $newvideo;
		  $output .= '<div class="no-records"><h3>No records found.</h3></div>';
		}
...
////Adding a custom page for admin
add_action( 'admin_menu', 'videos_library_menu' );

function videos_library_menu() {
	//add_menu_page( null, null, 'manage_options', 'admin-videos-library-page.php', 'videos_library_admin_page', '',  6 );
	 
	 // This is the hidden page
     add_submenu_page(
      null, 
      'videos-library hidden Page for admin',
      'videos-library hidden Page for admin', 
      'manage_options', 
      'admin-videos-library-page.php', 
      'videos_library_admin_page'
     );
}
function videos_library_admin_page(){
	 show_admin_bar(false);
	 // echo '<pre>' . print_r( $GLOBALS[ 'menu' ], TRUE) . '</pre>';
	?>
	
	<div class="wrap">
		<?php echo do_shortcode( '[posts_data_table sort_by="date" content_length=5 rows_per_page="15"]' );?>
	</div>
	<?php
