...
class EF_Settings extends EF_Module {
	...
	function action_admin_menu() {
		...
		add_menu_page( $this->module->title, $this->module->title, 'publish_pages', $this->module->settings_slug, array( $this, 'settings_page_controller' ), $this->module->module_url . $ef_logo ) ;
		...
		foreach ( $edit_flow->modules as $mod_name => $mod_data ) {
			if ( isset( $mod_data->options->enabled ) && $mod_data->options->enabled == 'on'
				...
				if ( $mod_name != 'user_groups' && $mod_name != 'custom_status' )
					...
					add_submenu_page( $this->module->settings_slug, $mod_data->title, $mod_data->title, 'publish_pages', $mod_data->settings_slug, array( $this, 'settings_page_controller' ) ) ;
		}
	}
	...
	function print_modules() {
		...
		if ( !count( $edit_flow->modules ) ) {
			...
			foreach ( $edit_flow->modules as $mod_name => $mod_data ) {
				...
				if ( !current_user_can('manage_options') && $mod_name != 'user_groups' && $mod_name != 'custom_status' )
					continue;
				...
				if ( $mod_data->options->enabled == 'off' || !current_user_can('manage_options') ) echo ' style="display:none;"';
