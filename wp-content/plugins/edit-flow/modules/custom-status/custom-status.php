...
class EF_Custom_Status extends EF_Module {
	...
	function add_custom_status( $term, $args = array() ) {
		...
		// Adding groups to statuses for notifications - Kelly - Feb 2015 - #1457
		$status_groups = get_option('edit_flow_status_groups');
		$status_groups['status-' . $response['term_id']] = $_POST['status-groups'];
		update_option('edit_flow_status_groups', $status_groups);
	...
	function update_custom_status( $status_id, $args = array() ) {
		...
		// Adding groups to statuses for notifications - Kelly - Feb 2015 - #1457
		$status_groups = get_option('edit_flow_status_groups');
		$status_groups['status-' . $status_id] = $_POST['status-groups'];
		update_option('edit_flow_status_groups', $status_groups);
	...
	function print_configure_view() {
		...
		<table class="form-table">
			...
			<tr class="form-field">
				<th scope="row" valign="top"><?php _e( 'Groups', 'edit-flow' ); ?></th>
				<td>
					<select name="status-groups[]" id="status-groups" multiple="multiple" style="width: 97%">
						<?php
						$selected_groups = get_option('edit_flow_status_groups')['status-' . $_GET['term-id']];
						if (empty($selected_groups)) {
							$selected_groups = [];
						}

						$user_groups = (new EF_User_Groups())->get_usergroups();
			...
			<div id="col-left">
				...
				<?php /** Custom form for adding a new Custom Status term **/ ?>
					...
					<div class="form-field">
						<label for="status-groups"><?php _e( 'Groups', 'edit-flow' ); ?></label>
						<select name="status-groups[]" id="status-groups" multiple="multiple" style="width: 97%">
							<?php
							$user_groups = (new EF_User_Groups())->get_usergroups();
							foreach ($user_groups as $group) {
