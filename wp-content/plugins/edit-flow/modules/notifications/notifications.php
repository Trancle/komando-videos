...
jQuery(document).ready(function($) {
	...
	function notification_status_change( $new_status, $old_status, $post ) {
		...
		if ( !in_array( $new_status, $ignored_statuses ) ) {
			...
			// Getting group by status - Kelly - Feb 2015
			$status_term_id = $edit_flow->custom_status->get_custom_status_by('slug', $new_status)->term_id;
			$groups = get_option('edit_flow_status_groups')['status-' . $status_term_id];
			
			$this->send_email( 'status-change', $post, $subject, $body, $groups );
		}
	 ...
	 * send_email()
	 */
	function send_email( $action, $post, $subject, $message, $groups, $message_headers = '' ) {
		global $edit_flow;
		...
		foreach ($groups as $group) {
			$users = (new EF_User_Groups())->get_usergroup_by('id', $group);

			foreach ($users->user_ids as $user) {
				$recipients[] = get_user_by('id', $user)->data->user_email;
			}
		}
