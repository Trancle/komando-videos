...
class EF_Calendar extends EF_Module {
	...
	function generate_post_li_html( $post, $post_date, $num = 0 ){
		...
		$featured_post = get_post_meta($post_id, 'feature_meta_id', true);
		?>
		<li class="<?php echo esc_attr( implode( ' ', $post_classes ) ); ?>" id="post-<?php echo esc_attr( $post->ID ); ?>">
			<div style="clear:right;"></div>
			<div class="item-static<?php if($featured_post) echo ' item-static-featured'; ?>">
				<div class="item-default-visible">
					<div class="item-status"><span class="status-text"><?php echo esc_html( $this->get_post_status_friendly_name( get_post_status( $post_id ) ) ) . '<br />' . get_the_time('', $post_id); ?></span></div>
					<div class="inner">
						<span class="item-headline post-title"><strong><?php echo get_post_type_object( get_post_type( $post->ID ) )->labels->singular_name . ': ' . esc_html( _draft_or_post_title( $post->ID ) ); ?></strong></span>
					</div>
				</div>
