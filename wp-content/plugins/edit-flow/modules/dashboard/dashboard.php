			...
			<table>
				...
				<tr>
					...
					<?php foreach($statuses as $status) : ?>
						...
						<td><center><?php echo esc_html( $status->name ); ?></center></td>
					<?php endforeach; ?>

				</tr>

				<?php
				// Starting custom post type loop - Kelly
				$args = array(
					'public'   => true,
					'_builtin' => false
				);
				...
				foreach ($post_types as $post_type) {
					...
					<tr>
						<td width="100"><?php echo $post_type->labels->name; ?></td>
						<?php $post_count = wp_count_posts( $post_type->name ); ?>
						<?php foreach($statuses as $status) : ?>
							<?php $filter_link = $this->filter_posts_link( $status->slug ); ?>

							<td width="100">
								<center><a href="<?php echo esc_url( $filter_link ) . '&post_type=' . $post_type->name; ?>"><?php $slug = $status->slug; echo esc_html( $post_count->$slug ); ?></a></center>
							</td>
